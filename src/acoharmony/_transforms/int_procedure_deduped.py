...
"""Pure Polars implementation of int_procedure_deduped."""
...
def execute(executor) -> pl.LazyFrame:
    """Deduplicate procedure codes using native Polars."""
    from acoharmony.medallion import MedallionLayer
    storage = executor.storage_config
    silver_path = storage.get_path(MedallionLayer.SILVER)
    stg = pl.scan_parquet(silver_path / "cclf3.parquet")
    ...
    result = (
        ...
        .select(
            [
                ...
                pl.col("clm_prcdr_cd").cast(pl.Utf8),
                pl.col("clm_prcdr_prfrm_dt").cast(pl.Utf8),

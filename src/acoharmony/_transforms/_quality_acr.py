...
class AllConditionReadmission(QualityMeasureBase):
    ...
    def calculate_denominator(
        ...
        # NOTE: REACH alignment (CMS PY2025 QMMR §3.1.2 p11 #2 'Patient is
        # actively aligned to a REACH ACO') is enforced upstream inside
        # AcrReadmissionExpression.identify_index_admissions at the
        # admission-month level — strictly tighter than a bene-level
        # any-month-of-PY filter. Don't re-apply it here; doing so would
        # have no effect (the inner join above is already a subset) but
        # would obscure where the spec rule lives.
        return denominator

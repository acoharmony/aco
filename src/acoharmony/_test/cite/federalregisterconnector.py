...
from acoharmony._test.cite.conftest import (
    _get_live_response_or_skip,
    _make_base_citation,
    _write_html,
)
...
def fr_document_html(fr_document_url: str, tmp_path: Path) -> Path:
    ...
    response = _get_live_response_or_skip(fr_document_url)
...
class TestFederalRegisterDetection:
    ...
    def test_can_handle_patterns(self) -> None:
        ...
        valid_urls = [
            ...
            "https://www.federalregister.gov/d/2024-25382/p-3",
            "https://federalregister.gov/d/2023-12345",
            "http://www.federalregister.gov/d/E9-12345",
        ]
        invalid_urls = [
            "https://example.com",
...
class TestFederalRegisterURLParsing:
    ...
    def test_parse_various_document_formats(self) -> None:
        ...
        test_cases = [
            ("https://www.federalregister.gov/d/2024-25382", "2024-25382"),
            ("https://www.federalregister.gov/d/E9-12345", "E9-12345"),
            ("https://www.federalregister.gov/d/2023-00001", "2023-00001"),
        ]
...
class TestFederalRegisterMetadata:
    ...
    def test_fetch_document_metadata_real(self) -> None:
        ...
        if metadata is None:
            pytest.skip("Live Federal Register API metadata unavailable")
...
class TestFederalRegisterParagraphExtraction:
    ...
    def test_extract_paragraph_from_xml(self, tmp_path: Path) -> None:
        ...
        xml_url = (
            "https://www.federalregister.gov/documents/full_text/xml/2024/12/09/2024-25382.xml"
        )
        xml_path = tmp_path / "2024-25382.xml"
        success = FederalRegisterConnector.download_xml(xml_url, xml_path)
        if not success:
            pytest.skip("Live Federal Register XML unavailable")
...
class TestFederalRegisterProcessing:
    ...
    @pytest.mark.integration
    def test_process_document_url_real_data(
        self, fr_document_url: str, fr_document_html: Path
    ) -> None:
        ...
        base_citation = pl.DataFrame(
            {
                "source_url": [fr_document_url],
                "title": ["Temp Title"],
                "url_domain": ["www.federalregister.gov"],
            }
        )
        citations = FederalRegisterConnector.process(
            fr_document_url, fr_document_html, base_citation
        )
        if citations is None:
            pytest.skip("Live Federal Register metadata unavailable")
    ...
    def test_process_paragraph_url_real_data(self, fr_paragraph_url: str, tmp_path: Path) -> None:
        ...
        response = _get_live_response_or_skip(fr_paragraph_url)
        html_path.write_bytes(response.content)
        base_citation = pl.DataFrame(
            {
                "source_url": [fr_paragraph_url],
                "title": ["Temp Title"],
                "url_domain": ["www.federalregister.gov"],
            }
        )
...
class TestFederalRegisterConnector:
    ...
    def test_can_handle_positive(self):
        ...
        assert (
            FederalRegisterConnector.can_handle("https://www.federalregister.gov/d/2024-25382")
            is True
        )
        assert (
            FederalRegisterConnector.can_handle("https://www.federalregister.gov/d/2024-25382/p-45")
            is True
        )
    ...
    def test_parse_url_with_paragraph(self):
        ...
        result = FederalRegisterConnector.parse_url(
    ...
    def test_construct_xml_url(self):
        ...
        assert (
            url
            == "https://www.federalregister.gov/documents/full_text/xml/2024/11/15/2024-25382.xml"
        )
    ...
    def test_extract_paragraph_text_by_data_number(self, tmp_path):
        ...
        html = (
            '<html><body><p class="paragraph" data-number="3">Paragraph 3 data.</p></body></html>'
        )
        html_path = _write_html(tmp_path, html)
    ...
    def _make_metadata(self, **overrides):
        defaults = {
            "title": "Medicare Final Rule",
            "type": "Rule",
            "abstract": "Summary of rule.",
            ...
            "docket_ids": ["CMS-1234"],
            "cfr_references": [{"title": 42, "part": 414}],
            "regulation_id_numbers": ["0938-AU99"],
            "html_url": "https://fr.gov/d/2024-25382",
            "pdf_url": "https://fr.gov/d/2024-25382.pdf",
        }
        defaults.update(overrides)
        return defaults

    @patch(
        "acoharmony._cite.connectors._federal_register.FederalRegisterConnector.fetch_document_metadata"
    )
    @pytest.mark.unit
    def test_process_document_only(self, mock_fetch, tmp_path):
        ...
        results = FederalRegisterConnector.process(
            "https://www.federalregister.gov/d/2024-25382", html_path, base
        )
    ...
    @patch(
        "acoharmony._cite.connectors._federal_register.extract_paragraph_by_id",
        return_value="XML paragraph text",
    )
    @patch(
        "acoharmony._cite.connectors._federal_register.FederalRegisterConnector.download_xml",
        return_value=True,
    )
    @patch(
        "acoharmony._cite.connectors._federal_register.FederalRegisterConnector.fetch_document_metadata"
    ...
    def test_process_with_paragraph_xml(self, mock_fetch, mock_dl, mock_extract, tmp_path):
        ...
        base = _make_base_citation()
        results = FederalRegisterConnector.process(
            "https://www.federalregister.gov/d/2024-25382/p-45", html_path, base
        )
    ...
    @patch(
        "acoharmony._cite.connectors._federal_register.extract_paragraph_by_id",
        side_effect=Exception("xml fail"),
    )
    @patch(
        "acoharmony._cite.connectors._federal_register.FederalRegisterConnector.download_xml",
        return_value=True,
    ...
    @pytest.mark.unit
    def test_process_paragraph_xml_fails_html_fallback(
        self, mock_fetch, mock_dl, mock_extract, tmp_path
    ):
        from acoharmony._cite.connectors._federal_register import FederalRegisterConnector
    ...
    @pytest.mark.unit
    def test_process_paragraph_xml_download_fails_html_fallback(
        self, mock_fetch, mock_dl, tmp_path
    ):
        from acoharmony._cite.connectors._federal_register import FederalRegisterConnector
    ...
    def test_process_no_metadata(self, mock_fetch, tmp_path):
        ...
        html_path = _write_html(tmp_path, "<html></html>")
        base = _make_base_citation()
        result = FederalRegisterConnector.process(
            "https://www.federalregister.gov/d/2024-25382", html_path, base
        )
    ...
    def test_process_no_document_number(self, tmp_path):
        ...
        base = _make_base_citation()
        result = FederalRegisterConnector.process(
            "https://www.federalregister.gov/about", html_path, base
        )
        assert result is None
    ...
    def test_process_paragraph_with_no_agencies(self, mock_fetch, mock_dl, mock_extract, tmp_path):
        ...
        base = _make_base_citation()
        results = FederalRegisterConnector.process(
            "https://www.federalregister.gov/d/2024-25382/p-10", html_path, base
        )
        assert len(results) == 2

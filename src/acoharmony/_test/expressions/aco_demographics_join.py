...
class TestAcoDemographicsJoin:
        ...
        df = pl.DataFrame(
            {
                ...
                "bene_lst_name": ["Doe"],
                "bene_mdl_name": ["A"],
                "bene_line_1_adr": ["123 Main St"],
                "geo_zip_plc_name": ["Springfield"],
    ...
    def test_build_zip5_expr_short_zip(self):
        ...
        result = df.select(build_zip5_expr())
        assert result["bene_zip_5"][0] == "00123"

    @pytest.mark.unit
    def test_build_zip5_expr_strips_punctuation(self):
        df = pl.DataFrame({"bene_zip": ["12-345", "12345-6789"]})
        result = df.select(build_zip5_expr())
        assert result["bene_zip_5"].to_list() == ["12345", "12345"]

    @pytest.mark.unit
    def test_build_zip5_expr_handles_empty_values(self):
        df = pl.DataFrame({"bene_zip": ["", None]})
        result = df.select(build_zip5_expr())
        assert result["bene_zip_5"].to_list() == [None, None]

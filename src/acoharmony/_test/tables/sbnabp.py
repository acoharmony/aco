...
from acoharmony._tables.sbnabp import Sbnabp
...
class TestSbnabp:
    ...
    @pytest.mark.unit
    def test_sbnabp_schema_fields(self) -> None:
        """Sbnabp has expected schema fields."""
        import dataclasses as dc

        fields = dc.fields(Sbnabp)
        field_names = [f.name for f in fields]
        expected = ['sheet_type']
        assert field_names == expected

    @pytest.mark.unit
    def test_sbnabp_data_types(self) -> None:
        """Sbnabp field types match expectations."""
        import dataclasses as dc

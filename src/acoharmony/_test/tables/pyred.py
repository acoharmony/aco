...
from acoharmony._tables.pyred import Pyred
...
class TestPyred:
    ...
    @pytest.mark.unit
    def test_pyred_schema_fields(self) -> None:
        """Pyred has expected schema fields."""
        import dataclasses as dc

        fields = dc.fields(Pyred)
        field_names = [f.name for f in fields]
        expected = ['sheet_type']
        assert field_names == expected

    @pytest.mark.unit
    def test_pyred_data_types(self) -> None:
        """Pyred field types match expectations."""
        import dataclasses as dc

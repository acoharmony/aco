...
from acoharmony._tables.sbqr import Sbqr
...
class TestSbqr:
    ...
    @pytest.mark.unit
    def test_sbqr_schema_fields(self) -> None:
        """Sbqr has expected schema fields."""
        import dataclasses as dc

        fields = dc.fields(Sbqr)
        field_names = [f.name for f in fields]
        expected = ['sheet_type']
        assert field_names == expected

    @pytest.mark.unit
    def test_sbqr_data_types(self) -> None:
        """Sbqr field types match expectations."""
        import dataclasses as dc

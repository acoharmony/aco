...
from acoharmony._tables.plaru import Plaru
...
class TestPlaru:
    ...
    @pytest.mark.unit
    def test_plaru_schema_fields(self) -> None:
        """Plaru has expected schema fields."""
        import dataclasses as dc

        fields = dc.fields(Plaru)
        field_names = [f.name for f in fields]
        expected = ['sheet_type']
        assert field_names == expected

    @pytest.mark.unit
    def test_plaru_data_types(self) -> None:
        """Plaru field types match expectations."""
        import dataclasses as dc
